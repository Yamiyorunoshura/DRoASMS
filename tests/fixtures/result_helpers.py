"""Result<T,E> error path test helpers for slash command testing.

Provides utilities for testing Result patterns including:
- Ok/Err assertion helpers
...
class ResultTestHelper:
    ...
    @staticmethod
    def assert_err_message_contains(result: Result[T, E], expected_substring: str) -> E:
        """Assert that a Result is Err and its message contains the expected substring."""
        error = ResultTestHelper.assert_err_result(result)
        assert (
            expected_substring in error.message
        ), f"Expected '{expected_substring}' in error message: {error.message}"
        return error

    @staticmethod
    def assert_err_context_has(result: Result[T, E], key: str, value: Any | None = None) -> E:
        """Assert that a Result is Err and its context has the expected key/value."""
        error = ResultTestHelper.assert_err_result(result)
        assert key in error.context, f"Expected key '{key}' in error context: {error.context}"
        if value is not None:
            assert (
                error.context[key] == value
            ), f"Expected context['{key}'] == {value}, got {error.context[key]}"
        return error
...
class DatabaseMockHelper:
    ...
    @staticmethod
    def create_database_error_result(message: str = "Database error") -> Err[Any, DatabaseError]:
        """Create a database error Result for testing."""
        return Err(DatabaseError(message))

    @staticmethod
    def create_connection_error_result(
        message: str = "Connection failed",
    ) -> Err[Any, DatabaseError]:
        """Create a connection error Result for testing."""
        return Err(DatabaseError(message, context={"reason": "connection_failed"}))
...
class PermissionMockHelper:
    ...
    def create_permission_check_result(
        ...
        return Err(
            PermissionDeniedError(error_message, context={"user_id": user_id, "guild_id": guild_id})
        )
    ...
    def create_permission_denied_result(
        message: str = "權限不足",
        user_id: int = 123456789,
    ) -> Err[Any, PermissionDeniedError]:
        """Create a permission denied Result for testing."""
        return Err(PermissionDeniedError(message, context={"user_id": user_id}))

    @staticmethod
    def create_validation_error_result(
        message: str = "驗證失敗",
        context: dict[str, Any] | None = None,
    ) -> Err[Any, ValidationError]:
        """Create a validation error Result for testing."""
        return Err(ValidationError(message, context=context or {}))


class ErrorPathTestHelper:
    """Helper for testing various error paths in slash commands."""

    @staticmethod
    def create_all_error_types() -> dict[str, Err[Any, Error]]:
        """Create instances of all standard error types for comprehensive testing."""
        return {
            "database": Err(DatabaseError("資料庫錯誤")),
            "validation": Err(ValidationError("驗證失敗")),
            "permission": Err(PermissionDeniedError("權限不足")),
            "generic": Err(Error("一般錯誤")),
        }

    @staticmethod
    def assert_error_response_sent(
        interaction: MagicMock,
        expected_message: str | None = None,
        ephemeral: bool = True,
    ) -> None:
        """Assert that an error response was sent to the interaction."""
        interaction.response.send_message.assert_called_once()
        call_args = interaction.response.send_message.call_args
        kwargs: dict[str, Any] = call_args[1] if call_args[1] else {}

        if ephemeral:
            assert kwargs.get("ephemeral", False) is True, "Error response should be ephemeral"
...
@pytest.fixture
def error_path_helper() -> ErrorPathTestHelper:
    """Provide ErrorPathTestHelper instance for tests."""
    return ErrorPathTestHelper()
