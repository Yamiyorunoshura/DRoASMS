...
async def test_propose_vote_execute_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _, status = await svc.vote(proposal_id=p.proposal_id, voter_id=100, choice="approve")
    _, status = await svc.vote(proposal_id=p.proposal_id, voter_id=101, choice="approve")
    assert status == "執行失敗"
