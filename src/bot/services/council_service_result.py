"""Result-based CouncilService implementation."""
...
class CouncilServiceResult:
    """Coordinates council governance operations using Result pattern."""
    ...
    async def create_transfer_proposal(
        ...
        *,
        ...
        if proposer_id == target_id:
            return Err(
                CouncilValidationError(
                    "Proposer and target must be distinct.",
        ...
        if target_department_id is None and target_id <= 0:
            return Err(
                CouncilValidationError(
                    "Either a valid target_id or target_department_id must be provided.",
        ...
        async with pool.acquire() as conn:
            ...
            if active >= 5:
                return Err(
                    ProposalLimitExceededError(
                        "There are already 5 active proposals for this guild. "
                        "Please complete or cancel existing proposals before creating new ones.",
            ...
            if voter_id not in snapshot:
                return Err(
                    VotingNotAllowedError(
                        ...
                        context={"voter_id": voter_id, "proposal_id": proposal_id},
                    )
                )
    ...
    async def _attempt_execution(
        ...
        if cfg is None:
            ...
            return Err(
                ExecutionFailedError(
                    "Missing council configuration at execution stage.",
                    context={"proposal_id": proposal.proposal_id, "guild_id": proposal.guild_id},
                )
            )
        ...
        if proposal.target_department_id is not None:
            ...
            if dept is None:
                error_msg = f"Department ID {proposal.target_department_id} not found."
                await self._gateway.mark_status(
                    connection,
                    proposal_id=proposal.proposal_id,
                    status="執行失敗",
        ...
        try:
            ...
            return Err(
                ExecutionFailedError(
                    f"Proposal execution failed: {error_msg}",
                    execution_error=error_msg,
                    context={"proposal_id": proposal.proposal_id, "error": error_msg},
                )
            )

        if isinstance(transfer_value_or_id, UUID):
            # Event pool mode - this shouldn't happen in sync mode
            error_msg = "Transfer returned UUID in sync mode."
            await self._gateway.mark_status(
                connection,
                proposal_id=proposal.proposal_id,
                status="執行失敗",
